  ...
  getTimeList() {
    ...
    if (time.hour == 0 && ref.read(orderProvider).endTodayTime.hour == 0) {
      while (time.hour <=
        ...
        listOfTime.add(
            "${time.hour}:${time.minute.toString().padLeft(2, '0')} - ${time.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).hour}:${time.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).minute > 30 ? time.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).minute : "00"}");
        time = time.addHour(1);
        if (time.hour == 0) {
          break;
        }
      ...
      while (time.hour <=
          (ref.read(orderProvider).endTodayTime.hour == 0
              ...
              : ref.read(orderProvider).endTodayTime.hour - 1)) {
        listOfTime.add(
            "${time.hour}:${time.minute.toString().padLeft(2, '0')} - ${time.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).hour}:${time.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).minute}");
        time = time.addHour(1);
        if (time.hour == 0) {
          break;
        }
    ...
    if (timeTomorrow.hour == 0 &&
      ...
      while (timeTomorrow.hour <=
        ...
        listOfTimeTomorrow.add(
            "${timeTomorrow.hour}:${timeTomorrow.minute.toString().padLeft(2, '0')} - ${timeTomorrow.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).hour == 24 ? "00" : timeTomorrow.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).hour == 24}:${timeTomorrow.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).minute.toString().padLeft(2, "0")}");
      ...
      while (timeTomorrow.hour <=
          (ref.read(orderProvider).endTomorrowTime.hour == 0
              ...
              : ref.read(orderProvider).endTomorrowTime.hour - 1)) {
        listOfTimeTomorrow.add(
            "${timeTomorrow.hour}:${timeTomorrow.minute.toString().padLeft(2, '0')} - ${timeTomorrow.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).hour == 24 ? "00" : timeTomorrow.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).hour + 1}:${timeTomorrow.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).minute.toString().padLeft(2, "0")}");
        timeTomorrow = timeTomorrow.addHour(1);
        if (timeTomorrow.hour == 0) {
          break;
        }
  ...
  List<String> getSingleDayTime(
    ...
    if (startTime.hour == 0 && endTime.hour == 0) {
      while (startTime.hour <= (endTime.hour == 0 ? 24 : endTime.hour - 1)) {
        listOfTime.add(
            "${startTime.hour}:${startTime.minute.toString().padLeft(2, '0')} - ${startTime.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).hour == 24 ? "00" : startTime.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).hour == 24}:${startTime.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).minute.toString().padLeft(2, "0")}");
      ...
      while (startTime.hour <= (endTime.hour == 0 ? 24 : endTime.hour - 1)) {
        listOfTime.add(
            "${startTime.hour}:${startTime.minute.toString().padLeft(2, '0')} - ${startTime.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).hour == 24 ? "00" : startTime.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).hour + 1}:${startTime.replacing(minute: int.tryParse(ref.read(orderProvider).shopData?.deliveryTime?.to ?? "40")).minute.toString().padLeft(2, "0")}");
        startTime = startTime.addHour(1);
        if (startTime.hour == 0) {
          break;
        }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            24.verticalSpace,
            state.currentIndexOne == 0 &&  ref.watch(orderProvider).isTodayWorkingDay
                ? ListView.builder(
                    shrinkWrap: true,
                    itemCount: 2,
